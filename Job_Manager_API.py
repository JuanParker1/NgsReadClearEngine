...
class Job_Manager_API:
    def __init__(self, max_number_of_process: int, upload_root_path: str, input_file_name: str, func2update_html):
        ...
        self.j_manager_thread_safe = Job_Manager_Thread_Safe(max_number_of_process, upload_root_path, input_file_name, self.__process_state_changed)
    ...
    def __process_state_changed(self, process_id, state):
        if state == State.Finished:
            try:
                send_email('mxout.tau.ac.il', 'TAU BioSequence <bioSequence@tauex.tau.ac.il>', 'edodotan@mail.tau.ac.il', subject=f'{process_id} process_id {state}.', content=f'http://localhost:8000/process_state/{process_id}')
            except:
